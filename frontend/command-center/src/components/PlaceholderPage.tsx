  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-5" style={{ borderBottom: '1px solid var(--border-hairline)' }}>
        <h1 className="t-title" style={{ color: 'var(--ink)' }}>{title}</h1>
        <p className="t-meta mt-0.5" style={{ color: 'var(--ink-3)' }}>{sub}</p>
      </div>
        ...
        <div style={{ maxWidth: 560 }}>
          ...
          >
            <div className="flex items-baseline justify-between">
              <h2 className="t-heading" style={{ color: 'var(--ink)' }}>In build</h2>
              <span className="badge badge-gray">v6</span>
            </div>
            <p className="t-body mt-2" style={{ color: 'var(--ink-3)', lineHeight: 1.6 }}>
              {description}
            </p>
            {links.length > 0 && (
